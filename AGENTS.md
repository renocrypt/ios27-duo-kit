...
- The glass probe keeps `/tmp/duo/glass-probe/` in `captures/<device>-<os>-<display>/<set>/` (iOS), `inspect/<date>/` (pictures to look at), `scratch/`, and `build/`, all rebuilt by `npm run probe` and `?calibrate`. Its report of residuals is ours: `app/tools/glass-probe/report.json`, next to the tokens it explains.

## Reference library (outside this folder)

`/tmp/duo/`: `duo-hig/` (HIG figures, the owner's reference images, raw research), `ios27-kit/` (frames read from Apple's iOS 27 UI Kit), `reference/` (Apple's AR model as GLB), `device-research/` (the USDZ). The dev server serves allowlisted files from it to the labs (`app/tools/vite-plugin-reference.ts`). Disposable: anything worth keeping is written down here, and what we make from these files (scripts, measurements, notes) lives here too. Three would be hard to get again if `/tmp` is cleaned: Apple's USDZ (its URL and hash are in `docs/research/2026-09-23-device-physical-reference/SOURCES.md`), the two status-bar SVGs exported by hand from the UI Kit (`ios27-kit/`), and the owner's two screenshots in `duo-hig/`.
