 ...
 * frosts more (the sizes scenes, captured on either display), the rim, and the hairline. It then writes the tokens, a report of every residual to
 * /tmp/duo/glass-probe/reports/<date>.json, and each tint's iOS and ours side by side to the page and to
...
export async function calibrate(root: HTMLElement, catalog: Record<string, CaptureSet>) {
  ...
  // ---- Write --------------------------------------------------------------------------------
  const name = String(report.date).slice(0, 19).replace(/[:T]/g, '-');
  ...
  const file = {
    $description: `Generated by app/labs/probe.html?calibrate ${from}. Do not edit; recapture (npm run probe) and recalibrate. Every value is [SIM]; the report with every residual is /tmp/duo/glass-probe/reports/${name}.json.`,
    material: { glass: {
  ...
  await fetch(`/@probe/report?name=${name}`, { method: 'POST', body: JSON.stringify(report) });
  row(root, `Report: /tmp/duo/glass-probe/reports/${name}.json. Writing tokens/glass.sim.tokens.json …`);
