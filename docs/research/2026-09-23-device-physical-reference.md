...
Confirmed present at `apple.com/iphone-duo/` ("View in your space"). The file name says Star White, but the file itself also contains a `Night_Sky` color variant (reviewer pass). Downloaded to `.references/apple-model/` (SHA-256 `5cab2ea63…8b2`, 8,107,136 bytes) and imported with `bpy.ops.wm.usd_import` in Blender 5.2 (headless; the sandbox's Metal probe crashes headless Blender before Python runs — this run required `dangerouslyDisableSandbox`, confirmed as a sandbox-caused crash, not a model problem).
