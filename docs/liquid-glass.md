...
To recalibrate (after an iOS update, or new scenes): `npm run probe` captures every set in `app/tools/glass-probe/scenes.json`; open `/labs/probe.html?calibrate`; look at `/tmp/duo/glass-probe/inspect/<date>/` and the report, `app/tools/glass-probe/report.json`; `npm run check`. Every scene fits the outer display's window, so a folded simulator can recapture them all; the material does not depend on the display.
...
The static material is done (decided September 25). What still differs from iOS shows only side by side over a grating, zoomed; in the presentation a screen's rim is a few pixels wide and the content is not a grating. These gaps are parked, known and measured, and are not worked on unless the presentation shows them:

- **The narrow frost at the rim.** Blurred after the lensing (the first refit's choice), it loses half the contrast in the outer 5 pt, where the lens squeezes the content; iOS keeps it, as a blur before the lensing does. At full size the two look alike. This loss also makes our phase unreadable at the rim, which is the calibration's large centre-line lensing error on the 220 × 96 capsule, not the lensing.
...
- **Values the fit cannot pin.** A recalibration may move them without changing the look or the residuals: clear glass's narrow σ at tint 1, where the wide share is 0.91 (8.9 and 10.4 pt fit alike, error 0.16), and the lensing parameters along their valley (the field stays at 0.76 pt rms).

Still guessed, and worth one probe pass each, because they show wherever the interface is on screen or moves:

- **Glyphs and labels on glass:** iOS adapts their colour to the content under the glass; ours are fixed CSS colours. Probe scenes with system controls (glass buttons, a toolbar, a tab bar) also answer whether controls flip between light and dark.
